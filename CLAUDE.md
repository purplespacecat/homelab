...
- **k3s re-defaults the local-path StorageClass on every restart** -- its deploy controller re-applies the packaged manifest with `is-default-class: "true"`, recreating the two-defaults ambiguity with nfs-client. Countered by `k8s/core/storage/local-path-storageclass.yaml` (Flux-owned, pins `"false"`); don't hand-patch the annotation, edit that file. SC spec fields are immutable -- the file must mirror the live spec exactly.
